...
| Where save images? | `{space}/content/images/{service}/` |
...
1. **Save image** to: `{space}/content/images/{service}/{YYYY}/{MM}/{DD}/image-{timestamp}.png`
...
:AI_OUTPUT: ~/Data/content/images/midjourney/2026/01/15/
:END:
...
**Archive:**
- `{space}/content/images/{service}/index.json` - Searchable index
...
```
Created archive directory: ~/Data/content/images/{service}/
```
...
1. **Preferred**: Call `plur_inject_hybrid` MCP tool with `prompt` = your task description and `scope` = `agent:image-generator`
